...
const SignUp = (): JSX.Element => {
  const { register, handleSubmit, errors, watch, setError, clearErrors, getValues } = useForm<IUser>({
    criteriaMode: "all",
    mode: "onChange",
                  ...
                  <Col md={6} xs={6}>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control
                        as="select"
                        name="region"
                        ref={register({ required: "Region" })}
                        isInvalid={errors?.region ? true : false}
                      >
                        <option value="">Select Region</option>
                        {Counties?.map((county) => (
                          <option value={county.city}>{county.city}</option>
                        ))}
                      </Form.Control>
